...
from prompt_learn.utils import InputFeatures, InputExample, get_verbalization_ids, chunks, trim_input_ids, \
    remove_final_punc, \
    lowercase_first
...
class TaskHelper(ABC):
    ...
    def train_step(self, batch: Dict[str, torch.Tensor], **kwargs):
        """
    ...
    def eval_step(self, batch: Dict[str, torch.Tensor], **kwargs):
        """
...
class SquadTaskHelper(TaskHelper):

    def add_special_input_features(self, input_example: InputExample, input_features: InputFeatures) -> None:
        positions = []
        multi_answers = input_example.meta["multi-answers"]
        for answer in input_example.meta["answers"]:
            answer_start = answer['answer_start']
            # start position should be adjusted to prompted input sequence
            if self.wrapper.config.wrapper_type == "mlm":
                start_position = len(self.wrapper.tokenizer.tokenize(input_example.meta["context"][:answer_start])) + 3
            else:
                start_position = len(self.wrapper.tokenizer.tokenize(input_example.meta["context"][:answer_start])) + 1
            end_position = len(self.wrapper.tokenizer.tokenize(answer["text"])) + start_position - 1
            start_position = min(start_position, self.wrapper.config.max_seq_length - 1)
            end_position = min(end_position, self.wrapper.config.max_seq_length - 1)
            positions.append([start_position, end_position])
        if not positions:
            positions.append([0, 0])
        if multi_answers:
            while len(positions) < 8:
                positions.append([self.wrapper.config.max_seq_length - 1, self.wrapper.config.max_seq_length - 1])
        input_features.meta["positions"] = positions

    def add_features_to_dict(self, features: List[InputFeatures], feature_dict: Dict[str, torch.Tensor]) -> None:
        feature_dict['positions'] = torch.tensor([f.meta['positions'] for f in features],
                                                 dtype=torch.long)

    def train_step(self, batch: Dict[str, torch.Tensor], **kwargs) -> Optional[torch.Tensor]:
        if self.wrapper.config.wrapper_type == 'sequence_classifier':
            return
        assert self.wrapper.config.wrapper_type == 'mlm' or self.wrapper.config.wrapper_type == "qa"
        inputs = self.wrapper.generate_default_inputs(batch)
        mlm_labels, labels, positions = batch['mlm_labels'], batch['labels'], batch['positions']
        input_ids = inputs["input_ids"]
        if self.wrapper.config.wrapper_type == 'mlm':
            prediction_scores = self.wrapper.model(**inputs)[0]
            logits = prediction_scores[mlm_labels >= 0]

            # compute classification loss by label mask score
            if self.wrapper.config.pattern_id == 0:
                cls_logits = logits[::3]
                cls_logits = torch.stack(
                    [self.wrapper.preprocessor.pvp._convert_single_mlm_logits_to_cls_logits(ml) for ml in cls_logits]
                )
                cls_loss = CrossEntropyLoss()(cls_logits, labels)

                # computing qa answer span loss by start&end position mask score
                start_logits = logits[1::3]
                end_logits = logits[2::3]
                start_logits = torch.gather(start_logits, 1, input_ids)
                end_logits = torch.gather(end_logits, 1, input_ids)
                start_loss = CrossEntropyLoss()(start_logits, positions[:, :, 0].squeeze(1))
                end_loss = CrossEntropyLoss()(end_logits, positions[:, :, 1].squeeze(1))
                position_loss = (start_loss + end_loss) / 2

                loss = 0.1 * cls_loss + position_loss
            else:
                cls_logits = logits[::2]
                cls_logits = torch.stack(
                    [self.wrapper.preprocessor.pvp._convert_single_mlm_logits_to_cls_logits(ml) for ml in cls_logits]
                ...
                # computing qa answer span loss by start&end position mask score
                start_logits = logits[1::2]
                start_logits = torch.gather(start_logits, 1, input_ids)
                start_loss = CrossEntropyLoss()(start_logits, positions[:, :, 0].squeeze(1))
                loss = cls_loss + start_loss
            return loss

        elif self.wrapper.config.wrapper_type == "qa":
            inputs['start_positions'] = positions[:, :, 0].squeeze(1)
            inputs['end_positions'] = positions[:, :, 1].squeeze(1)
            output = self.wrapper.model(**inputs, output_hidden_states=True)
            return output[0]
        else:
            raise NotImplementedError

    def eval_step(self, batch: Dict[str, torch.Tensor], **kwargs) -> Optional[
        ...
        inputs = self.wrapper.generate_default_inputs(batch)
        mlm_labels, labels, positions = batch['mlm_labels'], batch['labels'], batch['positions']
        start_positions, end_positions = positions[:, :, 0], positions[:, :, 1]
        input_ids = inputs["input_ids"]
        ...
        if self.wrapper.config.wrapper_type == 'mlm':
            if self.wrapper.config.pattern_id == 0:
                prediction_scores = self.wrapper.model(**inputs)[0]
                logits = prediction_scores[mlm_labels >= 0]
                start_logits = torch.gather(logits[1::3], 1, input_ids)
                end_logits = torch.gather(logits[2::3], 1, input_ids)
        else:
            output = self.wrapper.model(**inputs)
            start_logits, end_logits = output.start_logits, output.end_logits
        pred_answer_starts = np.argmax(start_logits.detach().cpu().numpy(), axis=1)
        pred_answer_ends = np.argmax(end_logits.detach().cpu().numpy(), axis=1)
        answers, prediction_answers = [], []
        for idx, (c_answer_start, c_answer_end) in enumerate(zip(start_positions, end_positions)):
            answer = []
            for (answer_start, answer_end) in zip(c_answer_start, c_answer_end):
                a = self.wrapper.tokenizer.decode(input_ids[idx][answer_start:answer_end + 1])
                a = self.normalize_answer(a)
                answer.append(a.split())
            answers.append(answer)
        for idx, (pred_answer_start, pred_answer_end) in enumerate(zip(pred_answer_starts, pred_answer_ends)):
            answer = self.wrapper.tokenizer.decode(input_ids[idx][pred_answer_start:pred_answer_end + 1])
            prediction_answers.append(self.normalize_answer(answer).split())
        return answers, prediction_answers
        # return answer_span_logits
...
class QuorefTaskHelper(SquadTaskHelper):
    def add_special_input_features(self, input_example: InputExample, input_features: InputFeatures) -> None:
        positions = []
        for answer in input_example.meta["answers"]:
            answer_start = answer['answer_start']
    ...
    def train_step(self, batch: Dict[str, torch.Tensor], **kwargs) -> Optional[torch.Tensor]:
        ...
        if self.wrapper.config.wrapper_type == 'mlm':
            ...
            # computing qa answer span loss by start&end position mask score
            start_logits = logits[0::2]
            end_logits = logits[1::2]
            start_logits = torch.gather(start_logits, 1, input_ids)
            end_logits = torch.gather(end_logits, 1, input_ids)
            start_loss = CrossEntropyLoss()(start_logits, positions[:, :, 0].squeeze(1))
            end_loss = CrossEntropyLoss()(end_logits, positions[:, :, 1].squeeze(1))
    ...
    def eval_step(self, batch: Dict[str, torch.Tensor], **kwargs) -> Optional[
        ...
        if self.wrapper.config.wrapper_type == 'mlm':
            prediction_scores = self.wrapper.model(**inputs)[0]
            logits = prediction_scores[mlm_labels >= 0]
            start_logits = torch.gather(logits[::2], 1, input_ids)
            end_logits = torch.gather(logits[1::2], 1, input_ids)
        else:
            output = self.wrapper.model(**inputs)
            start_logits, end_logits = output.start_logits, output.end_logits
        pred_answer_starts = np.argmax(start_logits.detach().cpu().numpy(), axis=1)
...
class SubjTaskHelper(SquadTaskHelper):
    pass


class DuorcTaskHelper(SquadTaskHelper):
    pass


class ConllFullTaskHelper(TaskHelper):
    def train_step(self, batch: Dict[str, torch.Tensor], **kwargs) -> Optional[torch.Tensor]:
        if self.wrapper.config.wrapper_type != 'token_classifier':
            return
        inputs = self.wrapper.generate_default_inputs(batch)
        inputs["labels"] = batch["labels"]
        return self.wrapper.model(**inputs)[0]
    ...
    def eval_step(self, batch: Dict[str, torch.Tensor], **kwargs):
        ...
        inputs = self.wrapper.generate_default_inputs(batch)
        input_ids = inputs["input_ids"]
        token_gt_batches = batch["labels"]
        token_prediction_batches = torch.argmax(self.wrapper.model(**inputs)["logits"], dim=2)
        entity_gts, entity_preds = [], []
        for (idx, token_predictions) in enumerate(token_prediction_batches):
            entity_preds_idxs = torch.where(token_predictions > 0)[0]
            entity_preds_classes = torch.gather(token_predictions, 0, entity_preds_idxs)
            entity_groups = self.split_entity_groups(entity_preds_idxs, entity_preds_classes)
            entity_pred = {}
            for entity in entity_groups:
                entity_text = self.wrapper.tokenizer.decode(input_ids[idx][entity[0]:entity[-1] + 1])
                entity_type = self.wrapper.config.label_list[token_predictions[entity[0]]].split("-")[-1]
                entity_pred[entity_text] = entity_type
            entity_preds.append(entity_pred)

        for (idx, token_gts) in enumerate(token_gt_batches):
            entity_idxs = torch.where(token_gts > 0)[0]
            entity_classes = torch.gather(token_gts, 0, entity_idxs)
            entity_groups = self.split_entity_groups(entity_idxs, entity_classes)
            entity_gt = {}
            for entity in entity_groups:
                entity_text = self.wrapper.tokenizer.decode(input_ids[idx][entity[0]:entity[-1] + 1])
                entity_type = self.wrapper.config.label_list[token_gts[entity[0]]].split("-")[-1]
                entity_gt[entity_text] = entity_type
            entity_gts.append(entity_gt)

        return entity_gts, entity_preds

    def split_entity_groups(self, idxs, classes):
        idx_groups = []
        if len(idxs) == 0:
            return idx_groups
        idx_groups.append([idxs[0]])
        top = 0
        for i in range(1, len(idxs)):
            if idxs[i] == idxs[i - 1] + 1 and classes[i] - classes[top] <= 1:
                idx_groups[-1].append(idxs[i])
            else:
                top = i
                idx_groups.append([idxs[i]])
        return idx_groups
