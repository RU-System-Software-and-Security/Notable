...
def save_predictions(path: str, wrapper, task_name: str, results: Dict, start: int, len: int):
    """Save a sequence of predictions to a file"""
    predictions_with_idx = []

    if wrapper.task_helpers[task_name] and wrapper.task_helpers[task_name].output:
        predictions_with_idx = wrapper.task_helper.output
    else:
        inv_label_map = {idx: label for label, idx in wrapper.preprocessors[task_name].label_map.items()}
        idxs, prediction_idxs = results['indices'][start:start + len], results['predictions'][start:start + len]
        for idx, prediction_idx in zip(idxs, prediction_idxs):
