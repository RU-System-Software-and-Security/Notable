...
from prompt_learn.utils import InputFeatures, InputExample, PLMInputFeatures
from prompt_learn.pvp import PVP, PVPS
...
class Preprocessor(ABC):
    ...
    def __init__(self, wrapper, task_name, pattern_id: int = 0, verbalizer_file: str = None):
        ...
        self.pvp = PVPS[task_name](self.wrapper, task_name, pattern_id, verbalizer_file)  # type: PVP
...
class SequenceClassifierPreprocessor(Preprocessor):
    ...
    def get_input_features(self, example: InputExample, **kwargs) -> InputFeatures:
        # inputs = self.wrapper.task_helper.get_sequence_classifier_inputs(example) if self.wrapper.task_helpers.values()[0] else None
        inputs = None
        if inputs is None:
