...
def main(args):
    ...
    for i, row in enumerate(scores):
        ...
        new_decoded = []
        for d in decoded:
            if "#" not in d and len(d) >= 7 and d not in new_decoded and re.match(pattern, d):
                new_decoded.append(d)
        logger.info(f"Top k for class {reverse_label_map[i]}: {', '.join(new_decoded[:25])}")
    ...
    for i in range(args.iters):
        ...
        for j, row in enumerate(scores):
            ...
            for d in decoded:
                if "#" not in d and len(d) >= 7 and d not in new_decoded and re.match(pattern, d):
                    new_decoded.append(d)
            logger.info(f"Top k for class {reverse_label_map[j]}: {', '.join(new_decoded[:25])}")
            results[str(reverse_label_map[j])] = new_decoded[:25]
    
    with open("../data/"+args.model_name+"_results.json", "w") as f:
        json.dump(results, f, indent=2)


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--device', default=0, help='Designated gpu device')
    ...
    parser.add_argument('--k', type=int, default=200, help='Number of label tokens to print')
